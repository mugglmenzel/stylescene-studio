...
export async function generateVideo(
  ...
  const projectId = 'veo-testing'; //process.env.GCP_PROJECT || await predictionServiceClient.getProjectId();
  const location = 'us-central1';

  // Using a video generation model.
  const endpoint = `projects/${projectId}/locations/${location}/publishers/google/models/veo-2.0-generate-001`;
  ...
  try {
    const [operation] = await predictionServiceClient.predict(request);
